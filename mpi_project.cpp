...
mutex queue_mutex, clock_mutex, stan_mutex, receiveResponses_mutex, m;
...
void intoLift(Narciarz narciarz){
	...
	randTime = 500 + (rand() % 2000);
	thread t1(sleepAndAnswer);
	...
	// Zejscie z wyciagu -> wyslij do wszystkich bcast '2' -> KONIEC ZJAZDU
	narciarz.stan = KONIEC_WJAZDU;
	//sendMessage();

	// Zjezdzaj na nartach + Odbieraj wiadomosci od procesow w watku
	randTime2 = 500 + (rand() % 2000);
	thread t2(sleepAndAnswer);
	std::this_thread::sleep_for(std::chrono::milliseconds(randTime2));
	t2.join();
	//zsynchronizuj zegar max((my,odebrane zegary)) i zinkrementuj go
	if(narciarz.zegar >= recivedClock)
	narciarz.zegar += 1;
